...
@try_export
def export_onnx_end2end(model, im, file, simplify, topk_all, iou_thres, conf_thres, max_wh, device, labels, kpt_label, prefix=colorstr('ONNX END2END:')):
    # YOLO ONNX export
    check_requirements('onnx')
    import onnx
    LOGGER.info(f'\n{prefix} starting export with onnx {onnx.__version__}...')
    ...
    #            batch_size,  topk_all,  batch_size,  topk_all, batch_size, topk_all, 15]
    # else:
    ...
    if kpt_label:
        output_names = ['output']
        shapes = [batch_size, topk_all, 22]
    else:
        output_names = ['output']
        shapes = [batch_size, topk_all, 7]
...
def run(
    ...
    if onnx_end2end:
        if isinstance(model, DetectionModel):
            labels = model.names
            f[2], _ = export_onnx_end2end(model, im, file, simplify, topk_all, iou_thres, conf_thres, max_wh, device, len(labels), kpt_label)
        else:
