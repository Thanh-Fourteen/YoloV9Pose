...
class IKeypoint(nn.Module):
    ...
    def __init__(self, nc=80, nkpt=5, ch=(), inplace=True):  # detection layer
        super().__init__()
        ...
        c2 = make_divisible(max((ch[0] // 4, self.reg_max * 4, 16)), 4) # channel
        c3 = max((ch[0], min((self.nc * 2, 128)))) # channels
        ...
        if self.nkpt:
            self.cvk = nn.ModuleList(nn.Conv2d(x, self.no_kpt, 1) for x in ch)

    def forward(self, x):
        shape = x[0].shape  # BCHW
        for i in range(self.nl):
            x_det = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
            if self.nkpt:
                x_kpt = self.cvk[i](x[i])
                x[i] = torch.cat((x_det, x_kpt), 1)
            else:
                x[i] = x_det
        if self.training:
            return x
        ...
        n_split = self.reg_max * 4 + self.nc + (self.no_kpt if self.nkpt else 0)
        raw_logits = torch.cat([xi.view(shape[0], n_split, -1) for xi in x], 2)

        if self.nkpt:
            box, cls, kpts = raw_logits.split((self.reg_max*4, self.nc, self.no_kpt), 1)
        else:
            box, cls = raw_logits.split((self.reg_max * 4, self.nc), 1)

        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides

        if self.nkpt:
            bs = shape[0]
            kpts = kpts.view(bs, self.nkpt, 3, -1)  # (bs, nkpt, 3, hw)
            ...
            grid = torch.stack((xv, yv), 2).view(-1, 2).float().to(device)  # (hw, 2)
            grid = grid.t().unsqueeze(0).unsqueeze(0)  # (1,1,2,hw)

            stride = self.strides.view(-1)  # (hw,)
            strides = stride.view(1, 1, -1)

            kpt_x = (kpts[:, :, 0, :].sigmoid() * 2 - 0.5 + grid[:, :, 0]) * strides
            kpt_y = (kpts[:, :, 1, :].sigmoid() * 2 - 0.5 + grid[:, :, 1]) * strides
            kpt_conf = kpts[:, :, 2, :].sigmoid()

            kpts = torch.stack((kpt_x, kpt_y, kpt_conf), dim=2).flatten(1, 2)

            y = torch.cat((dbox, cls.sigmoid(), kpts), 1)
    ...
    def bias_init(self):
        ...
        if self.nkpt:
            for k in m.cvk:
                k.bias.data[:] = 0.0  # init neutral cho keypoints
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, DualDetect, TripleDetect, DDetect, DDetectPose, DualDDetect, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic, IKeypoint)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, DDetect, DDetectPose, Segment, DSegment, Panoptic, IKeypoint)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, DSegment, Panoptic)) else self.forward(x)
            if isinstance(m, DDetectPose):
                fw = forward(torch.zeros(1, ch, s, s))[0]
                m.stride = torch.tensor([s / x.shape[-2] for x in fw])  # forward
            else:
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in {
            ...
            c2 = ch[f[-1]]
        # TODO: channel, gw, gd
        elif m in {Detect, DualDetect, TripleDetect, DDetect, DDetectPose, DualDDetect, TripleDDetect, Segment, DSegment, DualDSegment, Panoptic}:
